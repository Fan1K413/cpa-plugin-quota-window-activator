...
func managementRegistration() pluginapi.ManagementRegistrationResponse {
	return pluginapi.ManagementRegistrationResponse{Routes: []pluginapi.ManagementRoute{{Method: http.MethodGet, Path: managementPath, Description: "Read quota window activator status."}}, Resources: []pluginapi.ResourceRoute{{Path: "/status", Menu: "额度窗口激活器", Description: "Configure the activator and inspect quota-window state."}}}
}
...
<div class="title"><div><h1>额度窗口激活器</h1><div class="muted">配置 lazy reset 激活策略，并查看每个 credential + bucket 的运行状态。</div></div><span class="pill">v0.1.1</span></div>
...
const configPath='/v0/management/plugins/'+pluginID+'/config';
const providerSpecs=[['codex','Codex / ChatGPT OAuth',true],['antigravity','Antigravity',false],['claude','Claude',false],['gemini-cli','Gemini CLI',false],['kimi','Kimi',false],['xai','xAI / Grok',false]];
...
async function api(path,options){const opts=options||{};opts.cache='no-store';opts.headers=Object.assign({},opts.headers||{},{Authorization:auth()});if(opts.body)opts.headers['Content-Type']='application/json';const r=await fetch(path,opts);let body=null;const text=await r.text();if(text){try{body=JSON.parse(text)}catch(_){body=text}}if(!r.ok){const message=body&&body.message?body.message:(body&&body.error?body.error:'HTTP '+r.status);throw new Error(message)}return body}
function setNotice(el,text,kind){el.textContent=text;el.className='notice '+(kind||'')}
function renderProviderInputs(){const root=$('providers');root.innerHTML='';providerSpecs.forEach(spec=>{const id=spec[0],label=spec[1],canActivate=spec[2],box=document.createElement('div');box.className='provider';box.innerHTML='<label class="check"><input type="checkbox" id="provider-'+e(id)+'">'+e(label)+'</label><select class="mode" id="mode-'+e(id)+'"><option value="observe">仅观察</option>'+(canActivate?'<option value="activate_if_lazy">检测 lazy reset 后激活</option>':'')+'</select><span class="hint">'+(canActivate?'支持 credential-bound activation。':'当前版本只观察，不发送 activation。')+'</span>';root.appendChild(box)})}
function applyConfig(raw){currentConfig=raw&&typeof raw==='object'?JSON.parse(JSON.stringify(raw)):{};const c=Object.assign({},defaults,currentConfig);$('enabled').checked=c.enabled!==false;$('dryRun').checked=c.dry_run!==false;$('observationInterval').value=c.observation_interval||defaults.observation_interval;$('resetGracePeriod').value=c.reset_grace_period||defaults.reset_grace_period;$('verifyDelay').value=c.verify_delay||defaults.verify_delay;$('suppressionDuration').value=c.suppression_duration||defaults.suppression_duration;$('requestTimeout').value=c.request_timeout||defaults.request_timeout;$('maxRetries').value=c.max_retries||defaults.max_retries;$('maxConcurrency').value=c.max_concurrency||defaults.max_concurrency;$('stateDir').value=c.state_dir||'';$('disabledCredentials').value=Array.isArray(c.disabled_credentials)?c.disabled_credentials.join('\n'):'';const providers=c.providers||{};providerSpecs.forEach(spec=>{const id=spec[0],canActivate=spec[2],p=providers[id]||{};$('provider-'+id).checked=p.enabled!==false;const mode=canActivate&&(p.mode==='observe'||p.mode==='activate_if_lazy')?p.mode:(canActivate?'activate_if_lazy':'observe');$('mode-'+id).value=mode})}
